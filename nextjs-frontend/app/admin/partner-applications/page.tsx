...
export default function AdminPartnerApplications() {
    ...
    const fetchApps = async () => { try { setLoading(true); const res = await fetch(`${API}/applications`, { headers }); const r = await res.json(); if (r.success) setApps(r.data.filter((a: Application) => a.type === 'distributor' || a.type === 'super_stockist')); } catch { } finally { setLoading(false); } };
    ...
    return (
        <div className="admin-page max-w-7xl mx-auto space-y-6">
            <div><h1 className="text-2xl font-serif text-gray-900">Partner Applications ({apps.length})</h1><p className="text-sm text-gray-500">Distributor and super stockist partnership requests</p></div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-400"><div className="w-6 h-6 border-2 border-[#C38636] border-t-transparent rounded-full animate-spin mx-auto mb-2" />Loading...</td></tr>
                                : apps.length === 0 ? <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-400"><Handshake className="mx-auto mb-2" size={32} />No partner applications yet.</td></tr>
                                    : apps.map(a => (
                                        <tr key={a.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4"><span className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold uppercase ${a.type === 'distributor' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>{a.type?.replace('_', ' ')}</span></td>
                                            <td className="px-6 py-4 text-gray-600 text-xs">{a.city}{a.state ? `, ${a.state}` : ''}</td>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 text-center border border-gray-200">
                        ...
                        <p className="text-sm text-gray-500 mb-8">Delete <strong className="text-gray-700">{deleteConfirm.name}</strong>&apos;s partner application?</p>
